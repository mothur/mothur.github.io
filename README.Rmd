...
convert_mw_to_md <- function(mediawiki_file){
	...
	markdown_file <- str_replace(mediawiki_file, "_mw_files/(.*).mw", "wiki/\\1.md")
	...
	# add yaml material
	title <- str_replace(markdown_file, "wiki/(.*).md", "\\1") %>% str_replace_all(., "_", " ") %>% str_replace_all(., ":.*", "")
	md <- c(
						"---",
						paste0("title: '", title, "'"),
						"---",
...
read_tsv("_helper_files/keep_files.txt", col_name=c("file")) %>%
	mutate(convert = map(file, convert_mw_to_md))

file.rename("wiki/Main_Page.md", "pages/wiki.md")
```

## Manual fixing
* Find and replace < and > characters in mathjax code with \gt and \lt
* Change title of `wiki/index.md`



## To do...
* categories
  - Possible to make tags and have a page for each tag?


* Customize header nav bar

* convert wikimedia hyper links
  - get files that are linked in pages
  - post files to new location
  - create redirect pages from wiki path to new path
